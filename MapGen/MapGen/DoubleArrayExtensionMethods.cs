...
namespace MapGenerator
{
    public static class DoubleArrayExtensionMethods
    {
        ...
        public static IEnumerable<Point> GetNeighborCoordinates<T>(this T[,] doubleArray, int x, int y, int distance = 1, bool onlyXYaxis = false)
        {
        ...
        public static IEnumerable<T> GetNeighborContents<T>(this T[,] doubleArray, int x, int y, int distance =1, bool onlyXYaxis = false)
        {
            foreach (var position in doubleArray.GetNeighborCoordinates(x, y, distance, onlyXYaxis))
            {
        ...
        {
            return doubleArray.ContainsCoordinate(coordinates.X, coordinates.Y);
        }
        ...
        public static void FloodFill<T>(this T[,] doubleArray, Point startingPosition, Func<T[,], Point, bool> checkAndChangeTile, bool canMoveDiagonally = false, int iterations = int.MaxValue)
        {
            ...
            {
                foreach (var neighbor in doubleArray.GetNeighborCoordinates<T>(startingPosition.X, startingPosition.Y, 1, !canMoveDiagonally))
                {
